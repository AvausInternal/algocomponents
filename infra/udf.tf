...
resource "google_bigquery_routine" "get_known_users" {
  ...
  WHERE
    (_TABLE_SUFFIX BETWEEN START_SUFFIX
      AND END_SUFFIX)
    AND ((LOWER(ID_KEY)='user_id' AND user_id IS NOT NULL) OR (LOWER(up.key)=LOWER(ID_KEY) AND up.value.string_value IS NOT NULL))
  EOS
  arguments {
    name      = "ID_KEY"
    data_type = "{\"typeKind\" :  \"STRING\"}"
...
resource "google_bigquery_routine" "get_unknown_users" {
  ...
  *
  ...
  WHERE
    (_TABLE_SUFFIX BETWEEN START_SUFFIX
      AND END_SUFFIX)
  AND ((LOWER(ID_KEY)='user_id' AND user_id IS NULL) OR (LOWER(up.key)=LOWER(ID_KEY) AND up.value.string_value IS NULL))
  EOS
  arguments {
    name      = "ID_KEY"
    data_type = "{\"typeKind\" :  \"STRING\"}"
...
FROM (
  ...
  WHERE
    (_TABLE_SUFFIX BETWEEN START_SUFFIX
    AND END_SUFFIX)
QUALIFY ROW_NUMBER() OVER(PARTITION BY IF (LOWER(ID_KEY)='user_id',user_id, IF (LOWER(up.key)=LOWER(ID_KEY),up.value.string_value,user_pseudo_id)) ORDER BY event_timestamp DESC) = 1
) cte
EOS
  arguments {
    name      = "ID_KEY"
...
FROM (
  ...
  END
    ...
    ELSE
    'INCORRECT MODE SELECTED - CHOOSE FROM: (day, week, month)'
  END
    AS time_grouping
  FROM
  `${var.ga_project_id}.${var.ga_dataset_id}.events_*`
  ...
  WHERE
    ...
    AND (TIMESTAMP_MICROS(event_timestamp) BETWEEN TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL DAYS DAY)
      AND CURRENT_TIMESTAMP() )
    AND (_TABLE_SUFFIX BETWEEN FORMAT_TIMESTAMP('%Y%m%d',TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL DAYS DAY))
