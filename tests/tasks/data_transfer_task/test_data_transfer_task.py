...
class TestDataTransferTask(TestCase):
    ...
    SQLTask(
            sql_file_path=os.path.join("tests/tasks/data_transfer_task/sql/create_local_table.sql"),
    ...
    def test_correct_data_output(self):
        ...
        sql_adapter = LocalSqliteAdapter()
        sql_adapter.connect()
        assert sql_adapter.table_contains_columns("data_transfer_table_new",columns=["adapter","power_level"])
