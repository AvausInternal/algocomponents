...
class Dataset(GroupTask):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if input_table is None and feature_base is None:
            raise ValueError(
                "provide either an input_table or feature_base, got neither"
            )

        elif input_table is not None and feature_base is not None:
            raise ValueError("provide either an input_table or feature_base, got both")
            ...
            if (
                ...
                raise ValueError(
                    f"Featurebase must contain output_primary_keys or output_columns_created"
                )
            else:
    ...
    def primary_keys(self) -> List[str]:
        ...
        """
        primary_keys = []  # prevent changes to featurebase list object
    ...
    def _format_query_joins(
        self, feature: Feature, feature_number: int, pad: str
    ) -> str:
        """Helper function for _get_sql_join_query()
        ...
        join_table = [f"LEFT JOIN {feature.output_table} AS f{feature_number}"]
        join_condition = []

        for i, primary_key in enumerate(feature.output_primary_keys):
            if i == 0:
                on_condition = (
                    f"{pad}ON base.{primary_key} = f{feature_number}.{primary_key}"
                )
            else:
                # all but the first join need an AND prepended
                on_condition = (
                    f"{pad}AND base.{primary_key} = f{feature_number}.{primary_key}"
                )
    ...
    def _get_sql_join_query(
        ...
        Args:
            ...
            pad_spaces (int, optional): Number of SQL query indent spaces. Defaults to 2
    ...
    def shutdown(self):
        ...
        # drop all feature, and featurebase tables created during the run() process
